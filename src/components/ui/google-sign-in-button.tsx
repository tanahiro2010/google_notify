import { Box, ButtonBase, CircularProgress, Typography } from "@mui/material";

const GoogleLogo = () => (
  <Box
    component="svg"
    width={18}
    height={18}
    viewBox="0 0 48 48"
    aria-hidden
    sx={{ flexShrink: 0 }}
  >
    <path
      fill="#EA4335"
      d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"
    />
    <path
      fill="#4285F4"
      d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6.01c4.51-4.18 7.09-10.36 7.09-17.73z"
    />
    <path
      fill="#FBBC05"
      d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"
    />
    <path
      fill="#34A853"
      d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6.01c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"
    />
  </Box>
);

type GoogleSignInButtonProps = {
  onClick: () => void;
  disabled?: boolean;
  loading?: boolean;
  label?: string;
  loadingLabel?: string;
};

const GoogleSignInButton = ({
  onClick,
  disabled = false,
  loading = false,
  label = "Sign in with Google",
  loadingLabel = "Signing in...",
}: GoogleSignInButtonProps) => (
  <ButtonBase
    component="button"
    type="button"
    onClick={onClick}
    disabled={disabled || loading}
    sx={{
      display: "flex",
      alignItems: "center",
      justifyContent: "center",
      gap: 1.5,
      width: "100%",
      height: 40,
      px: 1.5,
      bgcolor: "#fff",
      border: "1px solid #dadce0",
      borderRadius: "4px",
      color: "#3c4043",
      fontSize: "14px",
      fontWeight: 500,
      letterSpacing: "0.25px",
      transition: "background-color 0.2s, box-shadow 0.2s, border-color 0.2s",
      "&:hover": {
        bgcolor: "#f8f9fa",
        borderColor: "#d2e3fc",
        boxShadow:
          "0 1px 2px rgba(60, 64, 67, 0.3), 0 1px 3px 1px rgba(60, 64, 67, 0.15)",
      },
      "&:active": {
        bgcolor: "#ecedee",
        boxShadow: "none",
      },
      "&.Mui-disabled": {
        bgcolor: "rgba(255, 255, 255, 0.38)",
        color: "rgba(60, 64, 67, 0.38)",
        borderColor: "rgba(31, 31, 31, 0.12)",
      },
    }}
  >
    {loading ? (
      <CircularProgress size={18} sx={{ color: "#4285F4" }} />
    ) : (
      <GoogleLogo />
    )}
    <Typography
      component="span"
      sx={{ fontSize: "14px", fontWeight: 500, lineHeight: 1, color: "inherit" }}
    >
      {loading ? loadingLabel : label}
    </Typography>
  </ButtonBase>
);

export { GoogleSignInButton };
